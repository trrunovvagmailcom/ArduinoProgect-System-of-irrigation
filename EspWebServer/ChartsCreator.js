...
$(document).ready(function() {
  let ServerIPWS = JSON.parse(GetHTTPCommandRespons("ip"));
  document.getElementById('ServerIP').innerHTML ="IP:" + ServerIPWS.ServerIP;
...
function ResponsManager(__Respons){
  ...
  if(__Respons.includes("{\"S\"","}")){
		UpdateTheParameters(__Respons);
...
//------------------------------------------------------------------------------
function GetHTTPCommandRespons(HttpCommand){
  let ServerHTTPRespons = $.get({
    url:`commands?command=${HttpCommand}`,
    async:false,
    data:{}
  }).responseText;
...
//document.getElementById('TimerPompSetButton').onclick = CustomeTimerPompJson();
//document.getElementById('but').onclick = multi;
